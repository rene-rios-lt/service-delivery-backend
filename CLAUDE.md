...
# Regenerate the canonical wire contract (contracts/openapi.json) after changing any REST DTO,
# endpoint, or response shape — then commit the result (ADR-0011). A plain build does NOT do this.
...
`contracts/openapi.json` is this repo's **committed source of truth for the REST wire contract** — the frontend and simulator mirror these shapes. It is generated by `Microsoft.Extensions.ApiDescription.Server` but **not** on an ordinary build (`OpenApiGenerateDocumentsOnBuild=false`), so a build never silently rewrites it and `OpenApiContractTests` can honestly compare the live document to the committed one.

- After changing any REST DTO, endpoint, or response shape, run `./scripts/regen-openapi.sh` and commit the updated `contracts/openapi.json`. `OpenApiContractTests` fails if you forget (the committed doc drifts from what the app serves).
- **OpenAPI covers REST only.** SignalR hub event payloads (e.g. `JobOfferReceived`) are **not** in `openapi.json` — their contract lives in the backend event DTOs + [`docs/api-design.md`](docs/api-design.md), and is guarded on the consumer side by captured-payload contract tests (QUAL-006).
